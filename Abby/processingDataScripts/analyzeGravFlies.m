...
for currMovNum = unique(movNumsList)
    ...
    if tripletCheck ~= length(camNamesList)
        ...
        bgFolder = fullfile(currFlyFolder,'bg');
        flyWindowParams = matchBackgroundsFunc(imgCell,bgFolder,cineFilenames);
        ...
        disp('Generating mp4...')
        cine2mp4(currFlyFolder,ExprNum,currMovNum,mp4Path)
    end
end
